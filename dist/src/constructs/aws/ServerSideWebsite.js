...
const _ServerSideWebsite = class extends import_abstracts.AwsConstruct {
  constructor(scope, id, configuration, provider) {
    ...
    const backendOriginPolicy = new import_aws_cloudfront.OriginRequestPolicy(this, "BackendOriginPolicy", {
      ...
      headerBehavior: import_aws_cloudfront.OriginRequestHeaderBehavior.all()
    });
    ...
    let albDomain = "";
    if (configuration.originType === "alb") {
      albDomain = import_aws_cdk_lib.Fn.getAtt(`${configuration.albLogicalId}`, "DNSName").toString();
    } else {
      ...
      apiGatewayDomain = import_aws_cdk_lib.Fn.join(".", [import_aws_cdk_lib.Fn.ref(apiId), `execute-api.${this.provider.region}.amazonaws.com`]);
    }
    ...
    this.distribution = new import_aws_cloudfront.Distribution(this, "CDN", {
      ...
      defaultBehavior: {
        origin: new import_aws_cloudfront_origins.HttpOrigin(configuration.originType === "alb" ? albDomain : apiGatewayDomain, {
